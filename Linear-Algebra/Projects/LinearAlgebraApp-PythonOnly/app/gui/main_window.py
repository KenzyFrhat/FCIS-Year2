...
from app.core.cofactor import cofactor_matrix

class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Linear Algebra App — REF / RREF / Transpose / Inverse / Cofactor')
        self.init_ui()
    ...
    def init_ui(self):
        ...
        self.btn_rref = QPushButton('RREF')
        self.btn_transpose = QPushButton('Transpose')
        self.btn_inverse = QPushButton('Inverse')
        self.btn_cofactor = QPushButton("Cofactor Matrix")

        for b in (self.btn_ref, self.btn_rref, self.btn_transpose, self.btn_inverse, self.btn_cofactor):
        ...
        self.btn_cofactor.clicked.connect(self.do_cofactor)

        # Removed invalid call: self.matrix_widget.fill_sample()
    ...
    def do_cofactor(self):
        try:
            A = parse_matrix_from_table(self.matrix_widget.table)

            C = cofactor_matrix(A)

            self.result_widget.show_matrix(C)

            steps = [
                "Cofactor matrix computed.",
                "Each entry C[i][j] = (-1)^(i+j) × det(minor(i,j))."
            ]
            self.steps_widget.set_steps(steps)
